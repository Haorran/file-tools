...
import { startDuplicate } from '../api';

const MODES = [
  { value: 0, label: '极速', desc: '仅匹配文件名' },
  { value: 1, label: '推荐', desc: '大小+首位局部哈希' },
  { value: 2, label: '完整', desc: '完整二进制校验' },
];
...
const scanDir = ref('');
const filterKB = ref('');
const filterUnit = ref('KB');
const error = ref('');

onMounted(() => {
  watch(() => task.triggerDuplicate.value, (v) => {
    if (v) {
      task.triggerDuplicate.value = false;
      handleSubmit();
    }
  });
});
...
async function handleSubmit() {
  error.value = '';
  if (!scanDir.value.trim()) { error.value = '请填写扫描目录路径'; return; }
  try {
    const filterValue = filterKB.value !== '' && filterKB.value !== null ? Number(filterKB.value) * (filterUnit.value === 'MB' ? 1024 : 1) : undefined;
    const bs = mode.value === 1 ? blockSize.value : undefined;
    const { taskId, sseUrl } = await startDuplicate({ mode: mode.value, scanDir: scanDir.value.trim(), filterKB: filterValue, blockSize: bs });
    task.connect(sseUrl, taskId, "duplicate");
    task.navigateToRunning('duplicate');
...
<template>
  <div class="flex flex-col">
    <h2 class="text-xl font-bold text-slate-800 dark:text-slate-100 mb-4 transition-colors shrink-0">🔍 目录内查重</h2>
    ...
    <div class="flex flex-col gap-4 mb-4 shrink-0">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5 transition-colors">扫描目录</label>
          <input v-model="scanDir" type="text" placeholder="/vol1/media"
        ...
        <div>
          <label class="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5 transition-colors">
            过滤阈值 <span class="font-normal text-slate-400 dark:text-slate-500">(可选)</span>
          </label>
          <div class="flex gap-2">
            <input v-model="filterKB" type="number" :placeholder="filterUnit === 'MB' ? '1（仅 ≥ 1MB 文件参与）' : '1024（仅 ≥ 1MB 文件参与）'" min="0"
              class="flex-1 px-3 py-2.5 rounded-lg border border-slate-300 dark:border-[#353842] bg-white dark:bg-[#1f252e] text-slate-800 dark:text-slate-100 font-mono text-sm placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200" />
            <select v-model="filterUnit"
              class="px-3 py-2.5 rounded-lg border border-slate-300 dark:border-[#353842] bg-white dark:bg-[#1f252e] text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200">
