...
> 接入 fnOS 开放 API 能力底座：应用内原生目录选择器选目录并授权、主题跟随系统、任务运行中离开提示、结果文件在文件管理器中打开。仅管理员可用。
...
- **三种校验模式**：极速 / 推荐 / 完整，兼顾速度与准确率
  ...
  | 模式 | 说明 | 分组方式 | 适用场景 |
  |------|------|---------|----------|
  | 极速 | 秒级完成，不读文件内容 | 按文件名分组（不要求同大小；可选忽略扩展名 / 忽略大小写） | 目录很大，只想快速按名粗筛重名文件 |
  | 推荐 | 兼顾速度与准确率，默认模式 | 按大小 + 首尾局部哈希分组（大小不同不归组） | 大多数日常场景：备份盘、下载目录、照片库等 |
  | 完整 | 全文件哈希校验，绝对可靠 | 两阶段：先 size+首尾局部哈希预分组，再对组内 ≥2 份文件做全文件哈希细分 | 重要数据整理，删除前需确认文件确实字节级相同 |
...
- **任务模型**：为避免冲突处理，一个目录同时只能有一个任务；如果暂存区不为空，处理后可再次扫描
...
- **单任务模型**：为避免冲突处理，一个目录（或目录对）同时只能有一个任务；暂存区不为空时需先处理，处理后可再次扫描
...
| 问题 | 回答 |
|------|------|
| 创建任务提示目录未授权？ | 创建任务时点「选择目录」走 fnOS 原生目录选择器选目录即自动授权；或到设置页查看 / 添加授权目录。旧系统（< 1.2.0401）需在系统设置手动授权。 |
| 删掉的文件还能恢复吗？ | 移入暂存区的文件可恢复（逐项或整组）；执行「永久删除」后不可恢复。 |
| 「检查更新」显示「暂无更新信息」？ | 开发期正常现象，不是故障。应用通过 GitHub Releases 查询新版本，未发布 Release 时即显示该提示。 |
| 关掉应用窗口，任务会停吗？ | 不会。任务在后台线程运行，关窗口不杀进程、不停任务；重新打开应用会自动恢复显示运行中 / 已完成的任务。 |
| 查重 / 比较很慢？ | 「极速」模式只按文件名分组，秒级完成；「推荐 / 完整」模式需要读取文件内容，机械盘或大目录较慢属正常。可先用「推荐」模式扫一个小目录验证流程。 |
...
[飞牛论坛主页](https://club.fnnas.com/home.php?mod=space&uid=64436) · [飞牛文件收集](https://s26.fnnas.net/s/01e4988bc5b4460d94)
